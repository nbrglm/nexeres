...
type ErrorResponse struct {
	...
	// Debug is a technical message that can be used for debugging.
...
func (e *ErrorResponse) Error() string {
	if opts.Debug {
		return fmt.Sprintf("Error %d: %s (Debug: %s)", e.Code, e.Message, e.Debug)
	}
	return e.Message
}

// TransformToResponse transforms the ErrorResponse to a version suitable for sending to clients.
// In non-debug mode, it removes the debug message to avoid exposing sensitive information.
func (e *ErrorResponse) TransformToResponse() *ErrorResponse {
	if opts.Debug {
		return e
	}
	// Ignore the debug message in non-debug mode
	return &ErrorResponse{
		Message:         e.Message,
		Debug:           "",
		Code:            e.Code,
